...
#get all room 
    ...
    def get_all_available_room_for_timeslot(self,timeslot_hour):
        rooms=timeslot_service.get_all_available_room_for_timeslot(timeslot_hour[0])
        
        for i in range(1,len(timeslot_hour)):
            rooms=set.intersection(rooms,timeslot_service.get_all_available_room_for_timeslot(timeslot_hour[i]))
    ...
    def get_all_available_room_with_timeslots_and_amenity(self,timeslot_hour,amenity_type):
        ...
        rooms_with_amenity=set.intersection(rooms_with_amenity,self.get_all_available_room_for_timeslot(timeslot_hour))
        return rooms_with_amenity
# #changes in room functions will be addedd later

    def book_room(self,room_id,booking_id,timeslot_hour):
        room=room_repository.get_room_by_id(room_id)
        if room!=None:
            room.delete_available_hour(timeslot_hour)
            room.add_booking(booking_id)

    def cancel_booking(self,room_id,booking_id,timeslot_hour):
        room=room_repository.get_room_by_id(room_id)
        if room!=None:
            room.add_available_hour(timeslot_hour)
